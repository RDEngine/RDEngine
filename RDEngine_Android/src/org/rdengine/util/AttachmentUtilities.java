...
public class AttachmentUtilities
{
    ...
    /**
     * The MIME type(s) of attachments we're willing to send via attachments. Any attachments may be added via Intents
     * with Intent.ACTION_SEND or ACTION_SEND_MULTIPLE.
     */
    public static final String[] ACCEPTABLE_ATTACHMENT_SEND_INTENT_TYPES = new String[]
    { "*/*", };
    /**
     * The MIME type(s) of attachments we're willing to send from the internal UI. NOTE: At the moment it is not
     * possible to open a chooser with a list of filter types, so the chooser is only opened with the first item in the
     * list.
     */
    ...
    {};
    /**
     * Filename extensions of attachments we're never willing to download (potential malware). Entries in this list are
     * compared to the end of the lower-cased filename, so they must be lower case, and should not include a "."
     */
    ...
    /**
     * Filename extensions of attachments that can be installed. Entries in this list are compared to the end of the
     * lower-cased filename, so they must be lower case, and should not include a "."
     */
    public static final String[] INSTALLABLE_ATTACHMENT_EXTENSIONS = new String[]
    { "apk", };
    /**
     * The maximum size of an attachment we're willing to download (either View or Save) Attachments that are base64
     * encoded (most) will be about 1.375x their actual size so we should probably factor that in. A 5MB attachment will
     * generally be around 6.8MB downloaded but only 5MB saved.
     */
    public static final int MAX_ATTACHMENT_DOWNLOAD_SIZE = (5 * 1024 * 1024);
    /**
     * The maximum size of an attachment we're willing to upload (measured as stored on disk). Attachments that are
     * base64 encoded (most) will be about 1.375x their actual size so we should probably factor that in. A 5MB
    ...
    /**
     * Return the filename for a given attachment. This should be used by any code that is going to *write* attachments.
     * This does not create or write the file, or even the directories. It simply builds the filename that should be
     * used.
     */
    ...
    /**
     * Return the directory for a given attachment. This should be used by any code that is going to *write*
     * attachments. This does not create or write the directory. It simply builds the pathname that should be used.
     */
    ...
    /**
     * Helper to convert unknown or unmapped attachments to something useful based on filename extensions. The mime type
     * is inferred based upon the table below. It's not perfect, but it helps.
     * 
     * <pre>
     *                   |---------------------------------------------------------|
     *                   |                  E X T E N S I O N                      |
    ...
    /**
     * @return mime-type for a {@link Uri}. - Use {@link ContentResolver#getType} for a content: URI. - Use
     *         {@link #inferMimeType} for a file: URI. - Otherwise throw {@link IllegalArgumentException}.
     */
    ...
    /**
     * Resolve attachment id to content URI. Returns the resolved content URI (from the attachment DB) or, if not found,
     * simply returns the incoming value.
     * 
     * @param attachmentUri
     * @return resolved content URI TODO: Throws an SQLite exception on a missing DB file (e.g. unknown URI) instead of
     *         just returning the incoming uri, as it should.
     */
    ...
    /**
     * sample: //content://com.android.email.attachmentprovider/1/36/RAW
     * //file:///data/data/com.android.email/databases/1.db_att/36
     * 
     * @param resolver
     * @param attachmentUri
     * @return
