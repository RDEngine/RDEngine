 ...
 * @author Alfred
 */
public class CollatorPinyin
{
    // / summary>
    // / 汉字拼音首字母列表 本列表包含了20902个汉字,用于配合 GetChineseSpell
    // 函数使用,本表收录的字符的Unicode编码范围为19968至40869
    // / /summary>
    private String strChineseFirstPY = "YDYQSXMWZSSXJBYMGCCZQPSSQBYCDSCDQLDYLYBSSJGYZZJJFKCCLZDHWDWZJLJPFYYNWJJTMYHZWZHFLZPPQHGSCYYYNJQYXXGJ"
    ...
    public String GetChineseSpell(String strText)
    {
        ...
        for (int i = 0; i < strText.length(); i++)
        {
            ...
            {
                myStr.append(String.valueOf(vChar));
            }
        }
        // //LOG.d("cccc", ""+myStr.toString());
        // //LOG.d("cccc", ""+myStr.toString().toUpperCase());
        return myStr.toString();
    }// GetChineseSpell

    public String GetFirstPinyin(String strText)
    {
        if (strText == null || strText.length() == 0)
            return strText;
        String myStr = null;
    ...
    public String AddFirstPinyin(String str)
    {
    ...
    private static CollatorPinyin cp = null;

    public static synchronized CollatorPinyin getInstance()
    {
        if (cp == null)
        {
            cp = new CollatorPinyin();
        }
        return cp;
    }

    public String getFirstSpell(String title, boolean b)
    {
        String ret = "";
        if (!b)
        {
            ret = GetChineseSpell(title);
        } else
        {
            ret = GetChineseSpell(title);
        }
     ...
     * @return
     */
    private String GetChineseSpellFull(String title)
    {
